...
| D9 | Paid inference is query-time only | [ADR-0009](adr/0009-local-open-embeddings.md) barred OpenAI during ingest | An opt-in curator step (`repair-corpus segment`) may call OpenAI to propose semantic boundaries for a selected document ([ADR-0048](adr/0048-semantic-knowledge-units.md)). `parse` and `ingest` stay key-free; embeddings stay local. | **Accepted, bounded** |
