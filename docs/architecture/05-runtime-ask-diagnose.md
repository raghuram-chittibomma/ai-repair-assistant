...
- **API:** `POST /v1/diagnose`, `/v1/diagnose/stream` with `session_id` ([ADR-0016](../adr/0016-http-api-docker.md)). JSON and the stream `done` event include a computed `tally` (first symptom, flattened cleared checks, `segments` per symptom, this turn’s offered checks, next or closed, session citation chip) for `/ui` ([ADR-0044](../adr/0044-diagnose-session-tally.md), [ADR-0045](../adr/0045-diagnose-tally-symptom-trail.md)). A corrected problem appends a span; it does not replace the first. The panel is read-only; it does not pick the next step.
